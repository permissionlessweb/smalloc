...
nextest_integration_tests! {
    ...
    fn large_realloc_down_realloc_back_up() {
        ...
        let p1 = unsafe { get_devsimpalloc!().alloc(l1) };
        assert!(!p1.is_null());
        let p2 = unsafe { get_devsimpalloc!().realloc(p1, l1, LARGE_SLOT_SIZE) };
        assert!(!p2.is_null());
        let p3 = unsafe { get_devsimpalloc!().realloc(p2, l2, LARGE_SLOT_SIZE * 2) };
        assert!(!p3.is_null());
    }

    fn test_alloc_1_byte_then_dealloc() {
        let layout = Layout::from_size_align(1, 1).unwrap();
        let p = unsafe { get_devsimpalloc!().alloc(layout) };
        assert!(!p.is_null());
        unsafe { get_devsimpalloc!().dealloc(p, layout) };
    }
...
fn help_test_multithreaded(threads: u32, iters: u64, dealloc: bool, realloc: bool, writes: bool)  {
    ...
    help_test_multithreaded_with_allocator(f, threads, iters, get_devsimpalloc!(), &mut tses);
